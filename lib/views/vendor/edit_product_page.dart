...
class _EditProductPageState extends State<EditProductPage> {
  ...
  // Observables
  final RxList<dynamic> _selectedImages =
      <dynamic>[].obs; // XFile or String (URL)
  ...
  /// تهيئة الـ Controllers بقيم المنتج الحالي
    ...
    if (widget.product.oldPrice != null &&
      ...
      final discount =
          ((widget.product.oldPrice! - widget.product.price) /
                  ...
                  100)
              .toInt();
      _discountController = TextEditingController(text: discount.toString());
    } else {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      // تحديد التصنيف والقسم الحالي
      if (widget.product.vendorCategoryId != null) {
        _selectedCategory.value = _categories.firstWhereOrNull(
          (cat) => cat.id == widget.product.vendorCategoryId,
        );
      }

      // تحديد القسم بناءً على نوع المنتج
      final sectionName = _getSectionNameFromType(widget.product.productType);
      _selectedSection.value = _sections.firstWhereOrNull(
  ...
  /// الحصول على اسم القسم من نوع المنتج
  String _getSectionNameFromType(String? type) {
    switch (type) {
      case 'offers':
        return 'offers';
      case 'sales':
  ...
  void _removeImage(int index) {
    final image = _selectedImages[index];
    if (image is String) {
      // إضافة URL للصور المحذوفة
      _deletedImageUrls.add(image);
    }
  ...
  void _openImagePreview(BuildContext context, {int? initialIndex}) {
    ...
    // تحويل جميع الصور إلى File
    List<File> imageFiles = [];
    for (var image in _selectedImages) {
      if (image is String) {
        // TODO: تحميل الصورة من URL وتحويلها لـ File
        continue;
      } else {
        imageFiles.add(File((image as XFile).path));
      }
    }
  ...
  Future<void> _saveProduct() async {
    ...
    try {
        ...
        if (image is String) {
          ...
          try {
            ...
            if (uploadResult['success'] == true) {
              ...
              debugPrint('New image uploaded: ${uploadResult['url']}');
            } else {
              throw Exception(
      ...
      debugPrint('All images ready: $allImageUrls');
      _uploadStatus.value = 'updating_product'.tr;
      ...
      debugPrint('=== ERROR UPDATING PRODUCT ===');
      debugPrint('Error: $e');
