...
import 'package:istoreto/controllers/vendor_product_search_controller.dart';
...
/// Modern Vendor Product Search Page
///
/// Features:
/// - Real-time search
...
/// - Responsive layout
class VendorProductSearchPage extends StatelessWidget {
  final String vendorId;

  const VendorProductSearchPage({super.key, required this.vendorId});
  ...
  Widget _buildSearchBar(VendorProductSearchController controller) {
    return Container(
  ...
  Widget _buildFilterSortBar(VendorProductSearchController controller) {
    return Container(
  ...
  Widget _buildFilterButton(VendorProductSearchController controller) {
    return Obx(() {
      final hasFilter = controller.selectedCategoryId.value != null;
      return OutlinedButton.icon(
        onPressed: () => _showCategoryFilter(controller),
  ...
  Widget _buildSortButton(VendorProductSearchController controller) {
    return Obx(() {
  ...
  Widget _buildActiveFilters(VendorProductSearchController controller) {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Wrap(
        spacing: 8,
        ...
        children: [
          ...
          if (controller.selectedCategoryId.value != null)
            Obx(() {
              final categoryName = controller.selectedCategoryName.value;
              return Chip(
                label: Text(
                  categoryName,
                  style: titilliumRegular.copyWith(
  ...
  Widget _buildProductsList(VendorProductSearchController controller) {
  ...
  Widget _buildEmptyState(VendorProductSearchController controller) {
    return Center(
      child: Column(
  ...
  void _showCategoryFilter(VendorProductSearchController controller) {
    Get.bottomSheet(
      SafeArea(
        child: Container(
              ...
              Flexible(
                child: Obx(() {
                  ...
                  return ListView.builder(
                    ...
                    itemCount: controller.vendorCategories.length,
                    itemBuilder: (context, index) {
                      final category = controller.vendorCategories[index];
  ...
  void _showSortOptions(VendorProductSearchController controller) {
    Get.bottomSheet(
      Container(
        decoration: const BoxDecoration(
          color: Colors.white,
  ...
  Widget _buildSortOption(
    VendorProductSearchController controller,
    String value,
    String label,
    IconData icon,
  ) {
